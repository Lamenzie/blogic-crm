...
const AdvisorDetail = () => {
  ...
  const { advisors, updateAdvisor, removeAdvisor } = useAdvisors();

  const advisor = advisors.find((a) => a.id === Number(id));
  ...
  if (!advisor || !formData) {
    return (
      <div className="p-6 bg-white rounded shadow">
        <h1 className="text-xl font-bold mb-4 text-darkBlue">Poradce nebyl nalezen</h1>
        <button
          onClick={() => navigate("/poradci")}
  ...
  const handleUpdate = () => {
    updateAdvisor(formData);
    setEditMode(false);
  };

  const handleDelete = () => {
    removeAdvisor(advisor.id);
    navigate("/poradci");
  };
  ...
  return (
    <div className="p-6 bg-white rounded shadow-md">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-darkBlue">Detail poradce</h1>
        <button
          onClick={() => navigate("/poradci")}
      ...
      <div>
        ...
        {editMode ? (
          ...
          <div className="space-y-2 text-gray-800">
            <p><strong>Jméno:</strong> {advisor.firstName} {advisor.lastName}</p>
            <p><strong>Email:</strong> {advisor.email}</p>
            <p><strong>Telefon:</strong> {advisor.phone}</p>
            <p><strong>Rodné číslo:</strong> {advisor.birthNumber}</p>
            <p><strong>Věk:</strong> {advisor.age}</p>
          </div>
        )}
      </div>
    </div>
